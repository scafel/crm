    ...
    <div class="panel admin-panel">
        ...
        <div class="padding border-bottom">
            <ul class="search" style="padding-left:10px;">
                ...
                <li>
                    <input type="text" id="test167"  name="time_id" class="input layui-input" style="width:250px; line-height:17px;display:inline-block" placeholder="年-月-日"/>
                    <a href="javascript:void(0)" class="button border-main icon-search" onclick="changesearch()" > 搜索</a></li>
                <li>
                    <a href="javascript:void(0)" class="button border-main icon-expand" onclick="dumpMessage()" > 导出本页数据</a></li>
                </li>
            </ul>
        </div>
            ...
            @else
                @foreach($time as $value)
                    ...
                    <tr>
                        ...
                        @foreach($user[$value->time_id] as $key=>$v)
                            <td><a href="javascript:findUser('{{ $value->time_id }}','{{ $number1++}}','channel')">{{ $v }}</a></td>
                        @endforeach
                        <td><a href="javascript:findUser('{{ $value->time_id }}',-1,'channel')">{{ $total_cos[$value->time_id] }}</a> </td>
                    </tr>
                @endforeach
            <tr>
                <td>合计</td>
                @foreach($number as $key=>$v)
                    <td><a href="javascript:findUser(-1,'{{ $number2++ }}','channel')">{{ $v }}</a></td>
                @endforeach
...
<script>
    ...
    function changesearch() {
        ...
        $.ajax({
            url:"{{ url('home/page/user/channelsearch') }}",
            type:'post',
            data:{value:value},
            success:function (res) {
                ...
                if (res.errorcode == 1){
                    ...
                    window.location.href = "{{ url('home/page/user/channellist') }}"+"?time_id="+res.data.time_id
                }else{
                    layer.closeAll("loading");
                    layer.alert("请求出错");
                }
