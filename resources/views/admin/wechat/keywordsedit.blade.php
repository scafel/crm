...
<div class="panel admin-panel">
    ...
    <div class="body-content">
        <form id="form" method="post" class="form-x" action="edit">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <input type="hidden" name="id" value="{{ $keywords->id }}">
            <input type="hidden" name="idtoken" value="{{ idMd5Token($keywords->id ) }}">
            <div class="form-group">
                <div class="label">
                    <label>微信端输入词：</label>
                </div>
                <div class="field">
                    <input type="text" class="input w50" value="{{ $keywords->keyname }}" name="keyname" data-validate="required:请输入关键词" readonly />
                    <div class="tips"></div>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="field">
                    <select name="returntype" class="input w50" onchange="changRturnType(this)">
                        ...
                        <option value="0" @if($keywords->returntype == 0 ) selected @endif>文本</option>
                        <option value="1" @if($keywords->returntype == 1 ) selected @endif>图片</option>
                        <option value="2" @if($keywords->returntype == 2 ) selected @endif>图文/链接</option>
                    </select>
                    <div class="tips"></div>
                </div>
            </div>
